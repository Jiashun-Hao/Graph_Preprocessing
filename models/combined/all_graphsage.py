...
def Model(File_Jsonl,File_Title):
    ...
    class GraphSAGE(nn.Module):
        def __init__(self, input_size, hidden_size, num_classes):
            ...
            self.conv1 = dgl.nn.SAGEConv(input_size, hidden_size, aggregator_type='mean')
            self.conv2 = dgl.nn.SAGEConv(hidden_size, num_classes, aggregator_type='mean')
    ...
    # Train
    def train_model(model, g, labels, epochs=3300, lr=0.01):
    ...
    # Evaluate
    def evaluate_model(model, g, labels):
        model.eval()
        with torch.no_grad():
            logits = model(g)
        ...
        print('GraphSAGE:'+File_Title+':'+f'Accuracy: {acc:.4f}, F1 Score: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}')
    ...
    model = GraphSAGE(input_size, hidden_size, num_classes)
    train_model(model, g_train, labels_train)

    g_test = create_graph(X_test, y_test)

    evaluate_model(model, g_test, labels_test)
